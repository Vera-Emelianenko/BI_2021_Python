...
def main(input_fastq, output_file_prefix, gc_bounds = [0, 100],
    ...
    if save_filtered == True:
        # I don't check for the directory here because output_file_prefix is the same and the file is created de novo anyways
    ...
    try:
        with open (input_fastq, 'r') as fastqfile:
            ...
            # set a counter of lines - needed to determine which line to check for quality (every 4th), which line is name and so on
            i = 0
            ...
            for line in fastqfile:
                ...
                if i % 4 == 0:
                    # if checker was not changed, so that all lines passed all filters, write those 4 lines to the output file
                    if checker == True:
...
def pass_gc_bounds(line, i, gc_bounds):
    ...
    if (2 + i) % 4 == 0:
        ...
        gc_content = 100*(line.count('G')+line.count('C'))/(line.count('T')+line.count('A')+line.count('G')+line.count('C'))
